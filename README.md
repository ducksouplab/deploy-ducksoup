...
- `ducksoup` service (only relevant variables are described here, please refer to the [Ducksoup documentation](https://github.com/ducksouplab/ducksoup#environment-variables) for an exhaustive list):  
    ...
    - `DS_FORCE_OVERLAY`: display a time overlay in videos (recorded)
...
Logs generated by DuckSoup (including errors, info... depending on log options set in environment variables, see above) are stored according to `DS_LOG_FILE`.

If a GStreamer crash happens, it won't be caught by DuckSoup but will be saved in `DS_CONTAINER_STDERR_FILE`. The format of this file is simply:
